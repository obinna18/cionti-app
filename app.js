...
const C = {
  ...
  navTo(screen) {
    ...
    if (screen === 'discover') {
      if (!S.mapInited) {
        // Map inside scroll div — init after a tick so DOM is ready
        setTimeout(() => { C.initMap(); }, 120);
      } else if (C.map) {
        setTimeout(() => C.map.invalidateSize(), 120);
      }
    }
  ...
  enterApp() {
    ...
    C.initCategories();
    C.listenNotifs();
    // Load skills first (seeds demo data), then load home feed sections
    C.loadSkills().then(() => {
      C.loadHomeFeed();
    });
  },
  ...
  renderHomeFeedSections() {
    ...
    C.renderOpportunities();
    C.renderFeaturedPros();
  },

  renderHomeStats() {
    const el = document.getElementById('home-stats');
    if (!el) return;
    const realCount = S.skills.filter(s => !s.id?.startsWith('demo')).length;
    const total = S.skills.length + Math.floor(Math.random() * 50) + 1200;
    const stats = [
    ...
    el.innerHTML = stats.map(s => `
      <div style="flex-shrink:0;background:var(--bg1);border:1px solid var(--border);border-radius:14px;padding:10px 14px;text-align:center;min-width:80px">
        <div style="font-size:1.3rem;margin-bottom:2px">${s.icon}</div>
        <div style="font-size:.95rem;font-weight:800;color:var(--p)">${s.val}</div>
        <div style="font-size:.62rem;color:var(--textm);margin-top:1px">${s.label}</div>
      </div>`).join('');
  },
  ...
  renderTrendingSkills() {
    ...
    const sorted = CATS.map(c => ({ ...c, count: catCounts[c.id] || Math.floor(Math.random()*20)+2 }))
      .sort((a,b) => b.count - a.count).slice(0, 10);
    el.innerHTML = sorted.map((c,i) => `
      <div style="flex-shrink:0;background:var(--bg1);border:1.5px solid ${i<3?c.color+'44':'var(--border)'};border-radius:14px;padding:10px 14px;cursor:pointer;min-width:90px;text-align:center;transition:all .2s" onclick="C.filterByCategory('${c.id}',null)">
        <div style="font-size:1.4rem;margin-bottom:4px">${c.label.split(' ')[0]}</div>
        <div style="font-weight:700;font-size:.75rem;color:var(--text)">${c.label.split(' ').slice(1).join(' ')}</div>
        <div style="font-size:.62rem;color:var(--textm);margin-top:2px">${c.count} near you</div>
        ${i<3?`<div style="width:4px;height:4px;border-radius:50%;background:${c.color};margin:4px auto 0"></div>`:''}
      </div>`).join('');
  },
  ...
  renderQuickActions() {
    ...
    if (!el) return;
    const actions = [
      { icon:'🗺', label:'Map View', color:'#2952FF', fn:"C.switchDiscoverMode('map')" },
      { icon:'🤖', label:'AI Match', color:'#8B5CF6', fn:"C.navTo('ai')" },
      { icon:'🛍', label:'Market', color:'var(--gold)', fn:"C.navTo('market')" },
      { icon:'✍️', label:'Post', color:'#00D97E', fn:"C.showCreatePicker()" },
    ];
    el.innerHTML = actions.map(a => `
      <div style="background:var(--bg1);border:1px solid var(--border);border-radius:14px;padding:12px 8px;text-align:center;cursor:pointer;transition:all .2s" onclick="${a.fn}" ontouchstart="this.style.transform='scale(.95)'" ontouchend="this.style.transform=''">
        <div style="font-size:1.5rem;margin-bottom:4px">${a.icon}</div>
        <div style="font-size:.72rem;font-weight:700;color:var(--text)">${a.label}</div>
      </div>`).join('');
  },
  ...
  renderActivityFeed() {
    ...
    if (!el) return;
    el.innerHTML = C.DEMO_ACTIVITY.map(a => `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--bg1);border:1px solid var(--border);border-radius:12px;margin-bottom:8px;cursor:pointer" onclick="C.navTo('feed')">
        ...
        <div style="flex:1;min-width:0">
          <div style="font-size:.83rem"><strong>${a.name}</strong> ${a.action}</div>
  ...
  renderOpportunities() {
    ...
    if (!el) return;
    const opps = [
      { title:'Photographer needed for wedding', loc:'Warri', budget:'₦30,000', urgent:true },
      { title:'Web developer for e-commerce site', loc:'Remote', budget:'₦80,000', urgent:false },
      { title:'Fashion designer for 50 uniforms', loc:'Effurun', budget:'Negotiate', urgent:true },
      { title:'Event MC for company dinner', loc:'Warri', budget:'₦25,000', urgent:false },
      { title:'Maths tutor for SS2 student', loc:'Effurun', budget:'₦5,000/hr', urgent:false },
    ];
    el.innerHTML = opps.map(o => `
      <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--bg1);border:1px solid var(--border);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:all .15s" onclick="C.navTo('feed')" ontouchstart="this.style.background='var(--bg2)'" ontouchend="this.style.background='var(--bg1)'">
        <div style="flex:1">
          <div style="font-weight:600;font-size:.85rem;margin-bottom:3px">${o.title}</div>
          <div style="font-size:.73rem;color:var(--textl)">📍 ${o.loc} · 💰 ${o.budget}</div>
        </div>
        ${o.urgent?'<span class="badge badge-deal" style="flex-shrink:0;font-size:.62rem">Urgent</span>':'<span style="color:var(--p);font-size:.85rem;flex-shrink:0">→</span>'}
      </div>`).join('') +
      `<button class="btn btn-o btn-full" style="margin-top:4px" onclick="C.navTo('feed')">See All Opportunities →</button>`;
  },

  renderFeaturedPros() {
    const el = document.getElementById('featured-pros');
    if (!el) return;
    const top = S.skills.filter(s => s.isAvailable).sort((a,b) => (b.averageRating||0)-(a.averageRating||0)).slice(0,8);
    el.innerHTML = top.map(s => `
      <div style="flex-shrink:0;width:120px;background:var(--bg1);border:1px solid var(--border);border-radius:16px;padding:14px 10px;text-align:center;cursor:pointer;transition:all .2s" onclick="C.viewProfile('${s.userId}')" ontouchstart="this.style.transform='scale(.97)'" ontouchend="this.style.transform=''">
        <div style="width:48px;height:48px;border-radius:50%;background:${C.avatarColor(s.userName||'?')};color:#fff;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:1.1rem;margin:0 auto 8px">${(s.userName||'?')[0].toUpperCase()}</div>
        <div style="font-weight:700;font-size:.75rem;margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(s.userName||'').split(' ')[0]}</div>
        <div style="font-size:.67rem;color:var(--textl);margin-bottom:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${s.title}</div>
        <div style="font-size:.7rem;color:var(--gold)">⭐ ${s.averageRating||'New'}</div>
        ${s.isAvailable?'<div style="width:8px;height:8px;border-radius:50%;background:var(--ok);margin:4px auto 0;box-shadow:0 0 6px var(--ok)"></div>':''}
      </div>`).join('');
  },

  async loadRemoteJobs() {
    const el = document.getElementById('remote-jobs-list');
    if (!el) return;
    // Demo jobs — Remotive API has CORS issues from browser, use static for now
    const jobs = [
      { title:'React Native Developer', company:'Paystack', location:'Remote · Nigeria', salary:'₦400k–800k/mo', tag:'🇳🇬' },
      { title:'Product Designer (UI/UX)', company:'Flutterwave', location:'Remote', salary:'$1,500–2,500/mo', tag:'💫' },
      { title:'Backend Engineer (Node.js)', company:'Kuda Bank', location:'Lagos or Remote', salary:'₦500k/mo+', tag:'🏦' },
      { title:'Digital Marketing Manager', company:'Jumia Nigeria', location:'Lagos', salary:'₦200k–350k/mo', tag:'📦' },
      { title:'Data Analyst', company:'MTN Nigeria', location:'Hybrid · Abuja', salary:'₦300k–450k/mo', tag:'📊' },
    ];
    el.innerHTML = jobs.map(j => `
      <div style="display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--bg1);border:1px solid var(--border);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:all .15s" onclick="C.navTo('feed')" ontouchstart="this.style.background='var(--bg2)'" ontouchend="this.style.background='var(--bg1)'">
        <div style="width:42px;height:42px;border-radius:10px;background:var(--bg2);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0">${j.tag}</div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:.85rem;margin-bottom:2px">${j.title}</div>
          <div style="font-size:.73rem;color:var(--textl)">${j.company} · ${j.location}</div>
        </div>
        <div style="font-size:.72rem;font-weight:700;color:var(--ok);white-space:nowrap;flex-shrink:0">${j.salary}</div>
      </div>`).join('') +
      `<button class="btn btn-o btn-full" style="margin-top:4px" onclick="C.navTo('feed')">See All Jobs →</button>`;
  },
  ...
  async loadDevArticles() {
    ...
    if (!el) return;
    const articles = [
      { title:'How to price your freelance services in Nigeria (2026 guide)', tag:'💰', read:'4 min' },
      { title:'Top 10 skills Nigerian companies are hiring for right now', tag:'🔥', read:'3 min' },
      { title:'How to get your first client as a designer or developer', tag:'🎯', read:'5 min' },
      { title:'Building a portfolio that wins clients in Africa', tag:'📁', read:'6 min' },
    ];
    el.innerHTML = articles.map(a => `
      <div style="display:flex;gap:12px;padding:12px 14px;background:var(--bg1);border:1px solid var(--border);border-radius:12px;margin-bottom:8px;cursor:pointer;transition:all .15s" ontouchstart="this.style.background='var(--bg2)'" ontouchend="this.style.background='var(--bg1)'">
        <div style="font-size:1.5rem;flex-shrink:0">${a.tag}</div>
        <div>
          <div style="font-weight:600;font-size:.84rem;line-height:1.5;margin-bottom:3px">${a.title}</div>
          <div style="font-size:.7rem;color:var(--textm)">${a.read} read · Cionti Tips</div>
        </div>
      </div>`).join('');
  },
  ...
  switchDiscoverMode(mode) {
    if (mode === 'map') {
      C.switchView('map');
      document.getElementById('cionti-map')?.scrollIntoView({ behavior:'smooth', block:'center' });
    } else if (mode === 'list') {
      C.switchView('list');
    }
  },
  ...
  showSearchMode() {
    // Scroll to show results when focused
  },
  hideSearchModeDelay() {
    setTimeout(() => {}, 300);
  },
  focusSearch() {
    document.getElementById('disc-search')?.focus();
  },
  ...
  async login() {
    ...
    if (!email || !pw) return C.showErr(errEl, '⚠️ Please enter both email and password');
    const btn = document.getElementById('btn-login');
    btn.disabled = true; btn.textContent = 'Signing in...';
    try {
      await auth.signInWithEmailAndPassword(email, pw);
      // success — onAuthStateChanged handles navigation
    } catch(e) {
      btn.disabled = false; btn.textContent = 'Sign In';
      // Specific actionable messages
      if (e.code === 'auth/user-not-found') {
        C.showErr(errEl, '❌ No account found with this email. Please sign up first.');
        // highlight sign-up link
        setTimeout(() => {
          document.querySelector('#s-login .auth-footer').innerHTML = '👇 <strong><span class="link-p" onclick="C.goTo(\'register\')" style="font-size:1rem">Create a free account now →</span></strong>';
        }, 500);
      } else if (e.code === 'auth/wrong-password') {
        C.showErr(errEl, '❌ Wrong password. Try again or click "Forgot password?"');
      } else if (e.code === 'auth/invalid-credential') {
        C.showErr(errEl, '❌ Email or password is incorrect. If you signed up with Google, use that button below.');
      } else {
  ...
  async register() {
    ...
    if (!city) return C.showErr(errEl, '⚠️ Enter your city');
    if (!pw || pw.length < 6) return C.showErr(errEl, '⚠️ Password must be at least 6 characters');
    if (!terms) return C.showErr(errEl, '⚠️ You must accept the Terms & Privacy Policy');

    // Generate OTP and store all reg data
    const otp = Math.floor(1000 + Math.random() * 9000).toString();
    S.pendingReg = {
      name, email, phone, address, city, state, pw,
      type: S.selectedType,
      gender: S.selectedGender || 'other',
      otp
    };

    // Show OTP on screen (since no email service yet)
    ...
    if (btn) { btn.disabled = true; btn.textContent = 'Code generated...'; }
    setTimeout(() => {
  ...
  async verifyOtp() {
    ...
    if (S.pendingReg && code === S.pendingReg.otp) {
      ...
      C.toast('Wrong code — check the code shown above', 'err');
    }
  },
  ...
  async sendPhoneCode() {
    ...
    if (!phone.startsWith('+')) {
      document.getElementById('ph-err').textContent = '⚠️ Enter number with country code e.g. +2348012345678 or start with 0';
    ...
    try {
      ...
      const msg = e.code === 'auth/invalid-phone-number' ? '❌ Invalid number. Use format: 08012345678 or +2348012345678' : e.message;
      document.getElementById('ph-err').textContent = msg;
      document.getElementById('ph-err').classList.add('show');
    }
  },
  ...
  switchView(view) {
    ...
    if (mv) mv.style.display = view==='map' ? 'block' : 'none';
    if (lv) lv.style.display = view==='list' ? 'block' : 'none';
    const rw = document.getElementById('radius-wrap');
    if (rw) rw.style.display = view==='map' ? 'flex' : 'none';
    const vtm = document.getElementById('vt-map');
    const vtl = document.getElementById('vt-list');
    if (vtm) vtm.classList.toggle('on', view==='map');
    if (vtl) vtl.classList.toggle('on', view==='list');
    if (view==='map' && !S.mapInited) C.initMap();
    else if (view==='map' && S.mapInited && S.map) setTimeout(() => S.map.invalidateSize(), 100);
...
Reply in 2-3 short paragraphs. Be helpful, conversational, and match Nigerian context. If relevant skills exist, mention them specifically. End with a quick action suggestion like "Want me to filter the map?" or "Should I show you their profiles?". Keep responses under 150 words.`;
  ...
  async publishContent() {
    ...
    let imageUrl = '';
    if (S.createImageFile && storage) {
      try {
        const ref = storage.ref(`content/${S.user.uid}/${Date.now()}`);
        await ref.put(S.createImageFile);
        imageUrl = await ref.getDownloadURL();
      } catch(e) { C.toast('Image upload failed — posting without image'); }
    }

    // Build the content doc
    const base = {
      type, userId: S.user.uid,
      userName: S.userData?.fullName || S.user.displayName || '',
      userCity: S.userData?.city || '',
      lat: S.userData?.lat || S.userLat,
      lng: S.userData?.lng || S.userLng,
      imageUrl, tags, visibility, location: S.createLocationTag || '',
    ...
    try {
      await db.collection('content').add(base);
      btn.disabled=false; btn.textContent='Publish';
      C.toast(`${type === 'post' ? 'Post' : type === 'shop' ? 'Product' : type === 'deal' ? 'Deal' : type === 'event' ? 'Event' : type === 'poll' ? 'Poll' : type === 'collab' ? 'Collab' : 'Content'} published! 🚀`, 'ok');
      S.createType = null; S.createImageFile = null;
      if (['shop','deal','event','collab'].includes(type)) {
        C.navTo('market');
      } else {
        C.navTo('feed');
        setTimeout(() => C.loadFeed('community'), 400);
      }
    } catch(e) {
      btn.disabled=false; btn.textContent='Publish';
      C.toast('Error publishing: ' + e.message, 'err');
    }
  },
